...
// Optional invite-key gate, read from a request header rather than anything the web login form
// sends -- this is deliberately invisible to index.html/the browser login UI, which stays
// completely unmodified. It exists for the separate desktop client (see the Electron project),
// which collects a server URL + key from the player before ever connecting and attaches
// X-Archon-Key to every request it makes. Unset by default: self-hosters (and this deployment,
// until an admin opts in) get the exact same open-registration behavior as before. Once set,
// register/login/the socket handshake all require the header -- in practice that means the
// ordinary web login page can no longer authenticate at all, since it has no way to send it;
// this is meant to be paired with the desktop client, not with public web access.
...
app.post("/api/register", (req, res) => {
  if (!keyOk(req.headers["x-archon-key"])) return res.status(403).json({ success: false, error: "Invalid or missing access key." });
...
io.on("connection", (socket) => {
  const providedKey = socket.handshake.headers && socket.handshake.headers["x-archon-key"];
